...
contract LaunchPadUpgradeable is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier validGasPrice() {
        require(tx.gasprice <= _maxGasPrice || _exemptions[msg.sender], "GP");
        _;
    }
    ...
    function initialize(
        ...
        _feeRate = 1;
        _maxGasPrice = 2000000000; // 2 gwei default
        _exemptions[msg.sender] = true;
    }
    ...
    function buyToken(address contractAddress) public payable validGasPrice nonReentrant returns (uint256) {
        ContractInfo storage info = contractInfo[contractAddress];
    ...
    function sellToken(address contractAddress, uint256 amount) validGasPrice public nonReentrant {
        ContractInfo storage info = contractInfo[contractAddress];
        require(info.saleIsActive, "SNA");
    ...
    function addGasPriceExemption(address account) external onlyOwner {
        _exemptions[account] = true;
    }

    function removeGasPriceExemption(address account) external onlyOwner {
        _exemptions[account] = false;
    }

    function isGasPriceExempt(address account) external view returns (bool) {
        return _exemptions[account];
    }

    // Upgrade configuration functions
    function setTargetFundRaisingAmount(uint256 newAmount) external onlyOwner {
        targetFundRasingAmount = newAmount;
    }

    function setFeeRate(uint8 newFeeRate) external onlyOwner {
        require(newFeeRate <= 10, "Fee rate too high"); // Max 10%
        _feeRate = newFeeRate;
    }
